...
export default function Room() {
    ...
    // Upgraded: Cleanly unmount, wipe connection logs, and reset both Agent and Client back to custom pre-join portals on disconnect [1.1, 1.1.1, 2.2, 4.1]
    const handleUserLeft = useCallback(() => {
        toast.error("Disconnection event registered.");

        // Sync connection-metadata to database before clearing variables [1, 2.3]
        if (socket.connected && activeConnectionIdRef.current) {
            socket.emit("leave-room", {
                roomId,
                email: agent ? agent.email : localStorage.getItem("user-email"),
        ...
        // ----------------------------------------------------------------
        // STRICT HARDWARE TEARDOWN: Physically stop all camera/mic tracks on disconnect [1.1.1, 2]
        // ----------------------------------------------------------------
        if (localStreamRef.current) {
            localStreamRef.current.getTracks().forEach(track => {
                track.stop();
        ...
        // Fallback: unmount active workspace [1.1.1]
        setIsJoined(false);
    ...
    }, [resetPeerConnection, socket, roomId, agent]); // Cleaned up dependencies safely using localStreamRef.current, removed localStream
    ...
    // Handler: Triggered when a new file is successfully sent/received locally [1]
    ...
    const handleLeaveRoom = () => {
        // Sync P2P connection-metadata to database before leaving room [1, 2.3]
        if (socket.connected && activeConnectionIdRef.current) {
            socket.emit("leave-room", {
                roomId,
                email: agent ? agent.email : localStorage.getItem("user-email"),
    ...
    const handleEndTicket = async () => {
        try {
            ...
            // Sync connection-metadata to database before closing ticket [1, 2.3]
            if (socket.connected && activeConnectionIdRef.current) {
                socket.emit("leave-room", {
                    roomId,
                    email: agent.email,
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <aside className="w-80 border-l border-slate-100 bg-white flex flex-col shrink-0 select-none hidden lg:flex">
                    ...
                    <ChatPanel
                        ...
                        isAgent={!!agent}
                        onFileLogged={handleFileTransferred} // Binds the file transfer metadata logging handler [1]
                    />
                </aside>
            </div>

...
export default function Room() {
    ...
    // Chat and File Sharing States
    const [chatInput, setChatInput] = useState("");
    const [messages, setMessages] = useState([]);
    const [filesTransferred, setFilesTransferred] = useState([]); // Tracks metadata of P2P file transfers

    // NEW STATE: Tracks remote peer's email locally to trigger Handshake Sync
    const [remoteEmail, setRemoteEmail] = useState(null);

    // Refs for tracking real-time connection metrics securely during unmount cleanups
    ...
    // SECURITY REF: Guarantees Client specs are transmitted strictly once per connection segment
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            socket.disconnect(); // Only disconnect socket on unmount
        };
    ...
    // AUTO-ATTACH HOOK: Auto-binds tracks whenever the peer connection resets
    useEffect(() => {
        if (peer && localStream) {
    ...
    // ----------------------------------------------------------------
    // HARDWARE CLEANUP UNMOUNT HOOK: Auto-stops and releases webcam, mic, and screen sharing
    // ----------------------------------------------------------------
    useEffect(() => {
        return () => {
    ...
    // Agent-Side Call Initiator Hook: Triggers the offer exchange strictly AFTER local stream is loaded AND client is present
    useEffect(() => {
    ...
    const handleIncomingCall = useCallback(async ({ from, offer }) => {
        ...
        setRemoteEmail(from); // Save agent email locally to keep state aligned
    ...
    // Upgraded: Cleanly unmount, wipe connection logs, and reset both Agent and Client back to custom pre-join portals on disconnect
    const handleUserLeft = useCallback(() => {
        toast.error("Disconnection event registered.");

        // Sync connection-metadata to database before clearing variables
        if (socket.connected && activeConnectionIdRef.current) {
            socket.emit("leave-room", {
                roomId,
                email: agent ? agent.email : localStorage.getItem("user-email"),
        ...
        recordedChunksRef.current = []; // Flush RAM
        ...
        // Fallback: unmount active workspace
        setIsJoined(false);

        // Client fall back cleanly to Waiting Lobby
        if (!agent) {
    ...
    useEffect(() => {
        ...
        socket.on("room-joined", async ({ role, clientToken }) => {
            if (role === "client") {
                ...
                // GUARD: Only initialize if media is not already active
                if (!localStreamRef.current) {
    ...
    // Handler: Triggered when a new file is successfully sent/received locally
    const handleFileTransferred = useCallback((fileMeta) => {
        setFilesTransferred((prev) => [...prev, fileMeta]);
    }, []);
    ...
    // Action: Client validates credentials locally first before connecting WebSocket
    ...
    // Case A: Render Client Waiting Lobby
    ...
    // Case C: Render Client Credentials Verification Form
    if (!isJoined && !agent) {
        return (
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <aside className="w-80 border-l border-slate-100 bg-white flex flex-col shrink-0 select-none hidden lg:flex">
                    ...
                    <ChatPanel
                        ...
                        isAgent={!!agent}
                        onFileLogged={handleFileTransferred}
                    />
                </aside>
            </div>

            {/* Agent Save/Discard Dialog Modal */}
